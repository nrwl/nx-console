...
const FIXTURES_PATH = path.normalize(
  path.join(__dirname, '..', 'fixtures', 'utf8-character.txt')
);
  ...
  describe('UnixTerminal', () => {
    describe('Constructor', () => {
      it('should set a valid pts name', () => {
        ...
        if (regExp) {
          assert.ok(
            regExp.test((<any>term)._pty),
            '"' + (<any>term)._pty + '" should match ' + regExp.toString()
          );
        }
      });
    });
