...
abstract class NxGenerateUiFile(name: String) : LightVirtualFile(name, NxGenerateUiFileType.INSTANCE, "") {
  init {
    isWritable = false
  }
...
class DefaultNxGenerateUiFile(name: String): NxGenerateUiFile(name) {
  ...
  fun setupGeneratorForm(generator: NxGenerator, options: List<NxGeneratorOption>) {
    onBrowserLoadEnd(browser) {
      ...
      query.addHandler { msg -> handleMessageFromBrowser(msg); null }
      val js = """
      ...
      postMessageToBrowser(StyleMessage(StylePayload(getHexColor(UIUtil.getPanelBackground()))))
