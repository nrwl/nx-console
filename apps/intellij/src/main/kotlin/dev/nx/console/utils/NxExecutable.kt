...
class NxExecutable {
    companion object {
        fun getExecutablePath(basePath: String): String {
            ...
            val nxExecutableName = if (SystemInfo.isWindows && !WslPath.isWslUncPath(basePath)) "nx.bat" else "nx"
            ...
            if (WslPath.isWslUncPath(binPath)) {
                return WslPath.parseWindowsUncPath(Paths.get(binPath, nxExecutableName).toString())!!.linuxPath
            }

            logger.info("Using ${binPath} as base to find local nx binary")
            val nxPackage =
                NpmPackageDescriptor.findLocalBinaryFilePackage(binPath, "nx")?.systemIndependentPath
