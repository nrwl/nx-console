...
fun parseRspackIncludePaths(rspackConfigFile: VirtualFile): List<String> {
    return computableReadAction {
        ...
        val createConfigCall = findCreateConfigCall(psiFile) ?: return@computableReadAction emptyList()

        val args = createConfigCall.arguments
        if (args.isEmpty()) return@computableReadAction emptyList()

        val firstArg = args[0] as? JSObjectLiteralExpression ?: return@computableReadAction emptyList()
        ...
        val includePathsArray =
            stylePreprocessorOptionsObj.findProperty("includePaths")?.value as? JSArrayLiteralExpression
                ?: return@computableReadAction emptyList()
