...
export class WorkspaceCodeLensProvider implements CodeLensProvider {
  ...
  async provideCodeLenses(document: TextDocument): Promise<CodeLens[]> {
    return this.cliTaskProvider
      .getProjectEntries()
      .map(([project, projectDef]: [string, ProjectDef]) => {
        return Object.entries(projectDef.architect || {}).map(
          ([target, targetDef]) => {
            ...
            const position = document.positionAt(
              findWorkspaceJsonTarget(document, project, { name: target })
            );
            return [
              ...
              ...configurations.map((configuration: string) => {
                const configurationPosition = document.positionAt(
                  findWorkspaceJsonTarget(document, project, {
                    name: target,
                    configuration,
                  })
                );
                const configurationArg =
