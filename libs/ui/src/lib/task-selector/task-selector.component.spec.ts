      ...
      tasks: [
        {
          ...
          task: {
            name: 'update',
            description:
              'Update one or multiple packages to versions, updating peer dependencies along the way.',
...
describe('TaskSelectorComponent', () => {
  ...
  it('should show collections of type "application" when query param is filter=application', fakeAsync(() => {
    ...
    expect(
      debugElement.querySelectorAll('mat-nav-list.task-list > mat-list-item')
        .length
    ).toBe(1);

    const collectionHtmlTitle: HTMLHeadElement | null = debugElement.querySelector(
      'mat-nav-list > div > h3'
    );
    ...
    if (collectionHtmlTitle) {
      ...
      throw new Error(
        'Element "mat-nav-list > div > h3" is not present in the DOM.'
      );
    }

    const taskHtmlTitle: HTMLHeadElement | null = debugElement.querySelector(
      'mat-nav-list.task-list > mat-list-item > h3'
    );
    ...
    if (taskHtmlTitle) {
      ...
      throw new Error(
        'Element "mat-nav-list > mat-list-item > h3" is not present in the DOM.'
      );
    }
  ...
  it('should show an empty collection of tasks when query param is filter=xxx', fakeAsync(() => {
    ...
    const taskList: HTMLHeadElement | null = debugElement.querySelector(
      'mat-nav-list'
    );
    expect(taskList).toBeNull();

    const noTaksHtmlContent: HTMLHeadElement | null = debugElement.querySelector(
      'div.no-tasks-container'
    );
    ...
    if (noTaksHtmlContent) {
      ...
      throw new Error(
        'Element "div.no-tasks-container" is not present in the DOM.'
      );
    }
