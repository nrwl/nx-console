...
You are in an nx workspace using Nx ${nxVersion?.full} and ${packageManager} as the package manager.
...
function runningTasksGuidelines(nxVersion: NxVersion | undefined) {
  if (!nxVersion) {
    return undefined;
  }

  const hasConsoleMessagingSupport =
    nxVersion.major === 0 || (nxVersion.major >= 21 && nxVersion.minor >= 1);

  if (!hasConsoleMessagingSupport) {
    return undefined;
  }
