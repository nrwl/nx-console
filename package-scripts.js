...
module.exports = {
  scripts: {
    ...
    dev: {
      'compile-server-and-electron': npsUtils.series.nps(withPlatform('compile'), withPlatform('copy-assets'), withPlatform('compile'), withPlatform('copy-server')),
      'prepare': npsUtils.series.nps(withPlatform('clean'), withPlatform('compile-server-and-electron'), withPlatform('pack'), 'frontend.build', withPlatform('copy-frontend')),
      'server': npsUtils.series.nps(withPlatform('compile-server-and-electron'), withPlatform('start-server')),
    ...
    publish: {
      ...
      'publish': npsUtils.series.nps('mac.prepackage', 'publish.builder-publish')
    },
    e2e: {
